(function (window, document) {
    var SliderBar = function (targetDom, options, check_code, name, pwd) {
        // 判断是用函数创建的还是用new创建的。这样我们就可以通过MaskShare("dom") 或 new MaskShare("dom")来使用这个插件了  
        if (!(this instanceof SliderBar)) return new SliderBar(targetDom, options, check_code, name, pwd);
        // 参数
        ...
        if (dataList.length > 0) {
            ...
            this.imgList = [];
            this.isSuccess = true;
            this.loginlog = '/groupapp/loginlogic/';
            this.name = name;
            this.pwd = pwd;
            this.lele = check_code;
            // console.log('1234');
            for (var i = 1; i < 10; i++) {
    ...
    SliderBar.prototype = {
        ...
        reToNewImg: function () {
            ...
            var imgSrc = "/static/image/" + _this.imgList[index] + "";
        ...
        cutImg: function () {
            ...
            // left 
            _this.resultX = Math.floor(0.3 * (_this.imgWidth - cutWidth * 2 - 4) + cutWidth);
            // top
            var cutTop = Math.floor(0.3 * (_this.imgHeight - cutHeight * 2) + cutHeight);
            // 设置样式
                ...
                if (_this.resultX > (left - 2) && _this.resultX < (left + 2)) {
                    ...
                    setTimeout(function () {
                        ...
                        $.ajax({
                            acycl: false,
                            type: "POST",
                            url: 'http://127.0.0.1:8000/groupapp/loginlogic/',
                            data: {name: _this.name, pwd: _this.pwd, code: _this.lele},
                            ...
                            success: function (msg) {
                                ...
                                if (msg === 'name_error') {
                                    location.href = '/groupapp/register/';
                                    alert("非法登录！")
                                }
                                if (msg === 'face') {
                                    location.href = '/groupapp/collectface/';
                                    alert("非法登录！")
                                }
                                if (msg === 'pwd_error') {
                                    location.href = '/groupapp/login/';
                                    alert("非法登录！")
                                }
                                if (msg === 'ok') {
                                    location.href = '/showapp/main/'
                                }
                            }
                        })
                    }, 600);
